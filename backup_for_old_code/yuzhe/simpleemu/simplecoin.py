...
class SimpleCOIN():
    ...
    class IPC(ABC):
        ...
        def __init__(self, func_map: dict, func_params_queues: list):
            ...
            self.func_params_queues = [mp.Queue() for _ in range(len(func_params_queues))]
            self.processes = {}  # 用于跟踪进程的字典
        ...
        def submit_func(self, pid: int, id: Any, args=(), kwargs={}):
            if pid < 0:  # use single-threading
                func = self.func_map[id]
                func(self, *args, **kwargs)
            elif id in self.func_map:
                if pid not in self.processes or not self.processes[pid].is_alive():
                    # 进程不存在或进程已结束，创建并启动新进程
                    def process_target(q):
                        while not q.empty():
                            func_id, args, kwargs = q.get()
                            self.func_map[func_id](self, *args, **kwargs)

                    # 为新进程指定队列
                    q = self.func_params_queues[pid]
                    process = mp.Process(target=process_target, args=(q,))
                    process.start()
                    self.processes[pid] = process  # 存储进程对象
                # 进程存在且正在运行，将任务添加到对应的队列
                self.func_params_queues[pid].put((id, args, kwargs), block=False)

        def is_func_running(self, pid: int) -> bool:
            # 检查指定pid的进程是否正在运行
