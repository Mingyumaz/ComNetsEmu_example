...
def main(simplecoin: SimpleCOIN.IPC, af_packet: bytes):
    global pro_num, num, random_forest_model
        ...
        if packet['Protocol'] == UDPNUM and packet['IP_src'] != node_ip:
            ...
            if header == HEADER_INIT:
                if simplecoin.is_func_running(pid=1) == False:
                    # 空闲状态，处理数据
                    simplecoin.submit_func(pid=1, id='rf_service', args=[af_packet, in_vnf_time])
                else:
...
@app.func('rf_service')
def rf_service(simplecoin: SimpleCOIN.IPC, af_packet: bytes, in_vnf_time: str):
    global num

    # print("rf_service start...")
